...
def main():
    ...
    if sabnzbd.WIN32 and service:
        handle_service_commands(service, sab_opts, serv_opts)
        exit_sab(0)
    ...
    if not autorestarted:
        ...
        notifier.send_notification(
            "SABnzbd%s" % notifier.hostname(), T("SABnzbd %s started") % sabnzbd.__version__, "startup"
        )
    ...
    while not sabnzbd.SABSTOP:
        ...
        if sabnzbd.WIN_SERVICE:
            rc = win32event.WaitForMultipleObjects(
                (sabnzbd.WIN_SERVICE.hWaitStop, sabnzbd.WIN_SERVICE.overlapped.hEvent), 0, 3000
            )
...
def handle_service_commands(service, sab_opts, serv_opts):
